...
const generateVerificationResult = (claimText, inputType, verificationMode) => {
  ...
  return {
    ...
    summaryTab: {
      forensicSummary: `Analysis of this ${inputType.toLowerCase()} reveals ${
        truthScore >= 70 ? "strong" : truthScore >= 40 ? "mixed" : "weak"
      } evidence support. The claim shows ${
        truthScore >= 70
          ? "high consistency"
          : truthScore >= 40
          ? "some inconsistencies"
          : "significant red flags"
      } across multiple verification checks.`,
    ...
    evidenceTab: {
      ...
      hallucinationGuardText:
        "Evidence suggested here may not perfectly match real articles. Always cross-check URLs and titles in your browser.",
    },
    biasSentimentTab: {
      corporateBiasLevel:
        truthScore >= 70 ? "LOW" : truthScore >= 40 ? "MEDIUM" : "HIGH",
      corporateBiasExplanation:
        truthScore >= 70
          ? "Minimal corporate spin detected. Content appears balanced."
          : truthScore >= 40
          ? "Some corporate framing present but balanced with other perspectives."
          : "Strong corporate bias detected with one-sided framing.",
      politicalLean: "CENTER",
      politicalLeanExplanation:
        "Content does not show strong partisan alignment in either direction.",
      ...
      emotionalTriggerExplanation:
        truthScore >= 70
          ...
          : truthScore >= 40
          ? "Some emotionally charged language present."
          : "Highly emotional language designed to provoke strong reactions.",
      biasChips: [
        {
          ...
          value:
            truthScore >= 70 ? "Low" : truthScore >= 40 ? "Medium" : "High",
          tooltip:
            "Assessment of corporate or brand-favorable framing in the content.",
        },
        {
        ...
        },
        {
          ...
          value:
            truthScore >= 70 ? "Low" : truthScore >= 40 ? "Medium" : "High",
          tooltip:
            "Degree to which language is designed to provoke emotional responses.",
        },
      ],
      saferAlternativeFraming: `A more neutral framing: ${claimText.substring(
        0,
        100
      )}${
        claimText.length > 100 ? "..." : ""
      } (with appropriate context and qualifiers added).`,
    },
    timelineTab: {
      ...
      guidelines: [
        {
          ...
          text:
            truthScore >= 70
              ? "Safe to share with standard context."
              : truthScore >= 40
              ? "Share with explicit caveats and uncertainties."
              : "Do not share without additional verification.",
        },
        {
        ...
        },
        {
          ...
          text:
            truthScore < 40
              ? "Be aware that this content may be designed to provoke anxiety or outrage."
              : "Standard emotional awareness applies.",
        },
      ],
    },
    auxiliary: {
      shortClaimSummary:
        claimText.length > 60 ? claimText.substring(0, 57) + "..." : claimText,
...
const getVerdictSummary = (score, inputType) => {
  if (score >= 85)
    return "High-quality sources with consistent reporting across multiple outlets.";
  if (score >= 70)
    return "Reputable sources support this claim, with minor uncertainties in details.";
  if (score >= 40)
    return "Mixed evidence with some supporting and some refuting sources.";
  if (score >= 20)
    return "Significant inconsistencies and questionable sourcing detected.";
...
const getKeyReasons = (score) => {
  if (score >= 70) {
    return [
      {
        type: "PRO",
        text: "Multiple tier-1 sources corroborate key facts with consistent details.",
      },
      {
        type: "PRO",
        text: "Timeline of events aligns across independent reporting.",
      },
      {
        type: "CAVEAT",
        text: "Some minor details may still be developing or subject to updates.",
      },
    ];
  } else if (score >= 40) {
    return [
      {
        type: "PRO",
        text: "Some reputable sources provide partial support for the claim.",
      },
      {
        type: "CON",
        text: "Significant gaps or contradictions exist in the evidence.",
      },
      {
        type: "CAVEAT",
        text: "Claims may be exaggerated or taken out of context.",
      },
    ];
  } else {
    return [
      {
        type: "CON",
        text: "No credible sources found to support the core claim.",
      },
      {
        type: "CON",
        text: "Evidence suggests fabrication or intentional misinformation.",
      },
      {
        type: "CAVEAT",
        text: "Content shows characteristics of coordinated disinformation.",
      },
    ];
  }
};
...
const getMockSources = (score) => {
  ...
  if (score >= 70) {
    sources.push({
      ...
      confidence: "HIGH",
      excerpt:
        "Multiple independent sources confirm the key facts with consistent reporting.",
  ...
  if (score < 40) {
    sources.push({
      ...
      confidence: "HIGH",
      excerpt:
        "Investigation reveals the claim contains fabricated details and misrepresented facts.",
...
const getTimelineEvents = (score) => {
  if (score >= 70) {
    return [
      {
        relativeLabel: "T-48h",
        description:
          "Story first reported by wire service with initial details.",
        linkedSourceIds: ["src_1"],
      },
      {
        relativeLabel: "T-24h",
        description:
          "Major news outlets pick up story with independent verification.",
        linkedSourceIds: ["src_1"],
      },
      {
        relativeLabel: "T-12h",
        description: "Additional sources provide corroborating evidence.",
        linkedSourceIds: ["src_1"],
      },
      {
        relativeLabel: "T-1h",
        description:
          "Story widely covered with consistent details across outlets.",
        linkedSourceIds: ["src_1"],
      },
    ];
  } else if (score >= 40) {
    return [
      {
        relativeLabel: "T-72h",
        description:
          "Claim first appears on social media from unverified account.",
        linkedSourceIds: [],
      },
      {
        relativeLabel: "T-24h",
        description: "Some outlets report claim with varying details.",
        linkedSourceIds: ["src_2"],
      },
      {
        relativeLabel: "T-12h",
        description: "Fact-checkers begin investigating inconsistencies.",
        linkedSourceIds: ["src_2"],
      },
      {
        relativeLabel: "T-1h",
        description: "Mixed conclusions from different verification sources.",
        linkedSourceIds: ["src_2"],
      },
    ];
  } else {
    return [
      {
        relativeLabel: "T-96h",
        description: "Claim originates from known disinformation source.",
        linkedSourceIds: ["src_3"],
      },
      {
        relativeLabel: "T-48h",
        description: "Spreads through social media with no credible sourcing.",
        linkedSourceIds: [],
      },
      {
        relativeLabel: "T-24h",
        description: "Fact-checkers identify claim as false.",
        linkedSourceIds: ["src_3"],
      },
      {
        relativeLabel: "T-1h",
        description: "Multiple authoritative sources refute the claim.",
        linkedSourceIds: ["src_3"],
      },
    ];
  }
...
export const verifyContent = async (
  ...
  try {
    ...
    // Map real API response to expected frontend format
    return {
      truthDial: {
        score: data.score,
    ...
    console.warn(
      "Real verification API failed, using mock data:",
      error.message
    );
