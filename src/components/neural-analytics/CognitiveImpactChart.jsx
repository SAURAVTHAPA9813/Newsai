...
const CognitiveImpactChart = ({ data }) => {
  ...
  const chartData = (data || []).map(item => ({
    ...item,
    averageAnxiety: item.avgAnxiety || item.averageAnxiety || 0  // Map avgAnxiety to averageAnxiety
  }));
      ...
      {chartData.length > 0 ? (
        ...
        <div className="h-[300px] flex items-center justify-center">
          <p className="text-sm text-text-secondary italic">
            No reading activity data available yet. Start reading to track your cognitive impact!
          </p>
        </div>
      )}
    </motion.div>
